...
class Messages_tests(unittest.TestCase):
    ...
    def test_get_most_recent_message_success(self, mock_get):
        ...
        mock_response_data = [
            {"created_timestamp": datetime.timestamp(datetime.now() - timedelta(days=2)), "message": "Old message"},
            {"created_timestamp": datetime.timestamp(datetime.now()), "message": "Recent message"}
        ]
    ...
    def test_post_new_message_success(self, mock_post):
        ...
        mock_post.return_value = Mock(ok=True, json=Mock(return_value=mock_response_data))
